...
from taxonomy import SEARCH_FAMILY_PROMPT_LIST
        ...
        try:
            ...
            response = self.client.models.generate_content(
                ...
                contents=[
                    ...
                    "Identify every distinct piece of furniture and lighting. Consolidate items seen from multiple angles "
                    "to avoid duplicates and use colours to differentiate between items. "
                    "Provide a short, unique, descriptive name for each item; when naming, prefer vocabulary consistent "
                    f"with furniture families {SEARCH_FAMILY_PROMPT_LIST}. "
                    "Return the result as a JSON array of strings (one string per item). "
                    "If the item appears twice, extract only one.",
                    *image_parts
                ],
                config= types.GenerateContentConfig(
                    temperature= 0.3,
